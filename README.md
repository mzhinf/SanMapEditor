...
- `Resources` 区会铺满右侧剩余高度，支持全量资源滚动查看，并采用窗口化渲染避免一次性挂载全部 DOM。
- `Resources` 图层可切换到 `byte08~byte11`，页面会按调色板叠加对应字段的点位分布，便于直接配合 `Record`/绘制工具编辑。
- bundle 导出时会同步读取同名 `.dor/.stg`，右侧 `据点` 面板可按据点高亮对应城门，并支持直接点中城池坐标或城门坐标联动选中。
