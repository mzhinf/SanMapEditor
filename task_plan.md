...
- [ ] 确认 `.stg` 层级块边界背后的直接字段与写回规则
- [ ] 命名城池块内人口、金、粮、士兵数量/兵种字段
- [ ] 确认 `.evt` 的对象/坐标/目标引用方式
- [ ] 继续从 `Emperor.exe` 确认 `.s/.x` 的真实生成与读取路径

## 最高优先级问题

1. `.stg` 势力块/城池块能否安全写回，以及直接 owner 字段是否存在
2. 城池块内人口、金、粮、士兵数量/兵种字段如何命名
...
1. 先把 `.stg` 层级块中的城池状态、士兵数量/兵种字段命名
2. 再设计 `.stg` 层级编辑与安全写回策略
