...
- **全复制**：全部选中 Cell 进入快照并携带完整字段；剪切后只保留 `acwx`。
- **非底层复制**：以 `acwy`、`acwz`、`terrain_tag`、`blocked`、`site_trigger`、`site_area` 是否存在为筛选条件，只复制除 `acwx`、`minimap_color` 外的字段；剪切时同样保留这两个排除字段。
...
1. 联动导入 `.m/.dor/.stg/.s/.x`、`stage.ini`、`History.txt` 和 `heads.dat`。
...
- `.s/.x` 继续采用“顶部派生、底部尾区保留”的兼容生成策略，等待更多游戏内样本验证。
