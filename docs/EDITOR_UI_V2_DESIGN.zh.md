...
- **非底层复制**：以 `acwy`、`acwz`、`terrain_tag`、`blocked`、`site_trigger`、`site_area` 是否存在为筛选条件，只复制除 `acwz`、`minimap_color` 外的字段；剪切时同样保留这两个排除字段。

左侧“数据叠加显示”和“合成对象 / 区域操作”标题栏提供收起/展开按钮。收起后只占一行，资源库获得更多可滚动空间。
