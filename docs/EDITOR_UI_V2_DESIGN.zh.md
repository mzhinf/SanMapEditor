...
2. 从 `stageNN.m` 联动导入同名 `.dor/.stg/.s/.x`、`stage.ini`、`History.txt` 和 `heads.dat`，并导出完整关卡 ZIP。
...
桌面启动器从“尚未导入地图项目”开始。用户选择 `stageNN.m` 或只含一个地图的完整目录后，校验器只读取显式目录并核对 `.m/.dor/.stg/.s/.x`、`stage.ini`、`History.txt`、`kingdom.cel` 与 `heads.dat`。状态进入“正在导入地图项目”，后台生成系统临时会话；完整成功后同一回环端口切换内容根并进入“地图项目已加载”。
...
导出前必须执行校验。校验失败时阻止生成归档；校验通过后固定下载单一 `stageNN-export.zip`，所有游戏文件、工作簿和 Patch 均置于 ZIP 根目录。
