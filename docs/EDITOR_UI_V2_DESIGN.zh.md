...
实现状态：核心编辑、管理、迁移和导出工作流已于 2026-07-13 落地；本文同时作为后续维护的界面与行为基线。
...
2. 从 `stageXX.m` 联动导入同名 `.dor/.stg/.s/.x`、`stage.ini`、`History.txt` 和 `heads.dat`，并导出完整关卡 ZIP。
...
| 区域 | 功能 |
| --- | --- |
| 文件 | 导入 `.m`，自动匹配同名 `.dor/.stg`，选择或复用 `heads.dat`；导出 `.m/.dor/.stg`；标题区显示创建者与实际打包日期。 |
...
- 平移、缩放、坐标定位。
- 鼠标悬停显示 cell 坐标和字段摘要。
- 单格选择，以及沿 2.5D 网格斜轴展开的 Shift 拖动等距框选；支持替换、添加、扣除和 Esc 取消。
- 图层叠加显示与透明度控制。
- 据点、城门、势力范围、核心区域高亮。
- 小地图定位和视口框显示。
...
导入分为桌面运行时导入和网页内项目续编两层。

桌面启动器从“尚未导入地图项目”开始。用户选择 `stageXX.m` 或只含一个地图的完整目录后，校验器只读取显式目录并核对 `.m/.dor/.stg/.s/.x`、`stage.ini`、`History.txt`、`kingdom.cel` 与 `heads.dat`。状态进入“正在导入地图项目”，后台生成系统临时会话；完整成功后同一回环端口切换内容根并进入“地图项目已加载”。

失败时显示“地图项目导入失败”及完整原因；已有旧会话时继续提供旧会话。重新选择资源必须确认替换，关闭启动器清理当前会话。

网页顶部“导入”只用于同一桌面会话中的项目续编。载入新 `.m` 前再次确认替换浏览器状态；它不会重新生成另一套资源、头像或 Sidecar 尾区。
...
区域复制、区域剪切和合成对象共用稀疏快照结构，但复制与剪切使用临时剪贴板。操作范围直接来自画布多选，不提供“设起点”。选区使用 `u = col + ceil(row/2)`、`v = floor(row/2) - col` 的等距坐标；快照相对位置保存为 `du/dv`，确保复制目标跨越奇偶行时视觉轮廓不发生半格错位。
...
| 状态 | 含义 | 可用操作 |
| --- | --- | --- |
| `empty` | 服务根指向发布包空入口。 | 选择 `.m`、选择目录、打开状态页、退出。 |
| `importing` | 正在校验并生成临时 Bundle。 | 禁止并发导入；退出等待事务结束后清理。 |
| `failed` | 本次导入失败并显示原因。 | 重选资源；有旧会话时仍可打开旧会话。 |
| `loaded` | 当前临时会话完整可用。 | 打开编辑页、确认后重选、导出、退出。 |

`LauncherRuntimeController` 驱动状态转换。导入在线程中执行，结果通过队列回到 Tk 主线程；HTTP 服务固定监听同一本机端口，每个新请求从受锁保护的当前内容根取文件。
...
用户选择 → 校验
  ...
  └─ 成功：生成临时目录
      ├─ 失败：删除半成品，保留 current
      └─ 成功：切换 current，清理 previous
```

会话元数据记录输入路径、大小、SHA-256、创建时间、场景编号和导出结果。只有会话根直接子目录且带专用标记的目录可以自动删除。

### 10.3 浏览器编辑状态

浏览器状态包含 `meta/resources/image/minimap`、原始 Cell 与 Patch、撤销重做栈、本地参考文件、场景与公共母表、城门、管理 Patch、当前选区与活动对象。

领域层从当前临时 Bundle 派生，不替代原始二进制。写回必须落到明确字段；`.s/.x` 尾区必须来自当前用户会话，缺失或长度异常时导出阻断。
...
1. 无资源启动、四态桌面导入、显式同组文件校验、事务临时会话和固定回环端口切换。
2. 等距地图渲染、四类图层编辑、数据叠加、小地图、坐标定位和资源预览。
3. 撤销/重做、区域复制与剪切、两种复制模式、合成对象保存与粘贴。
4. 势力、据点、城门和武将的增删改查、归属同步、地图定位和头像显示。
5. `stage.ini` 母表与 `History.txt` 候选武将合并、Raw 恢复、Patch 跨版本导入。
6. 导出前关联校验，以及包含游戏文件、工作簿和 Patch 的单一 ZIP 导出。
7. 发布包五文件白名单、目录/ZIP/TOC 资源审计，以及运行时禁止回退仓库数据目录。

仍保留以下边界：

- `heads.dat` 只作为头像资源读取，不开放字段级写回。
- `.stg` 未确认字段、运行时字段和尾区按原始字节保留，不根据 UI 状态猜测重算。
- `.s/.x` 采用“顶部从 `.m` 派生、底部保留用户原始尾区”的兼容生成策略；缺失或长度异常时阻止导出，不做零填充。
- STG 并行研究实现的统一收口暂时搁置，正式编辑模型仍以 KSY 与 `editor_model.py` 为准。
