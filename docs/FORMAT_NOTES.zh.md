...
### 5.6 2026-07-08 STG 字段修订补充

本轮根据 42 个 `.stg` 样本的字段统计和 `Emperor.exe` 静态反汇编，修正以下旧结论：

- `entity_part1 +0x00..+0x20` 是固定零保留/填充区，不是运行时所属势力。
- `entity_part1 +0x30` 是运行时势力/AI 阵营候选，但并非总等于父 Force 序号。
- `site_part2 +0x27C..+0x28C` 控制五个可选 Entity；`+0x2AC` 在当前样本固定为 0，不再作为额外 Entity 数量解析。
- `site_part2 +0x058..+0x130` 是 AI/行动模板参数表，`+0x134..+0x20C` 是固定零保留带，`+0x210..+0x278` 是运行时状态尾字段组。
- `after_forces_tail` 不是填充；当尾区大于 `0xA0` 时，最后 `0xA0` 字节作为 trailer 候选，其前为尾部对象区。尾部对象区可检测到 entity-like block，但当前仍缺少可靠显式 count，必须原样保留。
