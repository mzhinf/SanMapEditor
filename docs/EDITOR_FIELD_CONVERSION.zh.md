...
已有行按 `fieldLocations[表][row_id][列].byteOffset` 原位写回。新增行号与业务索引分离，城池标题取当前 `site_name`，未直接编辑的数值列从合法模板继承。新增武将按 228 字节完整块插入普通武将区末尾，并更新文件起始的 `main_count`；新增城池按 100 字节双块插入城池区末尾，并在主块插入位移后的 `city_count` 位置更新计数。两类记录均不做步长补齐，后续未知块逐字节保留。
