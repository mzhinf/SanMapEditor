...
本文档是项目文档的统一目录，说明每份文档负责什么、当前状态以及何时需要更新。临时分析和任务计划应放在已忽略的 `.planning/`，不再把过程流水长期保留在仓库根目录。
...
| 原文件 | 处理方式 | 信息去向 |
| --- | --- | --- |
| `task_plan.md` | 删除 | 当前能力写入 README 和编辑器设计；临时计划改放 `.planning/` |
| `findings.md` | 删除 | 格式结论进入格式笔记，数据链路进入数据管理和字段转换 |
| `progress.md` | 删除 | 重要里程碑摘要进入项目历史和 CHANGELOG |
