...
“据点 / 新增据点”不使用一组固定经济常量。创建顺序如下：

1. 优先寻找尚未被当前场景 `city_index` 使用的 `stage.ini` 城池行，并继承该行名称、类型、规模、人口、金钱、粮草及全部已知和保留字段。
2. 没有可用母表行时，依次选择“房屋属性为 0 且城池规模为 1”“房屋属性为 0”“第一条城池行”作为模板，并复制整行字段。
3. 完全没有模板时，房屋属性回退为 0（城池）、城池规模回退为 1；人口、金钱、粮草、待命士兵、开发、商业、治安和太守回退为 0。
4. 据点坐标始终取新增时当前选中的地图 Cell，所属势力取当前势力；没有可用母表行时，都市索引和母表行号分别取现有最大值加一，名称为 `Site N`。
5. 导出 `.stg` 时运行坐标、父势力序号和全局据点序号仍按上面的派生规则重建。
