...
- [x] 删除 38 个旧 `tools` Python 转发文件，正式命令统一迁移到 `python -m san_tools`。
...
- [x] 执行完整测试和 wheel 内容验证。
- [x] 检查 `git status --short`，确保仅包含预期源码与文档。
- [x] 扫描令牌、私钥、本机路径和误提交的大文件。
