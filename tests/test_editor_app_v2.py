...
class TestEditorAppV2Template(unittest.TestCase):
    ...
    def test_editor_app_uses_ksy_m_field_names(self) -> None:
        ...
        self.assertIn('house_attr: 1', html)
        self.assertIn('siteHouseAttrValue(site) === 1', html)
        self.assertIn("if (kind === 'site') return scenarioRows('site').filter(site => siteHouseAttrValue(site) === 1);", html)
