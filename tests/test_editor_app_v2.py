...
class TestEditorAppV2Template(unittest.TestCase):
    ...
    def test_editor_app_uses_ksy_m_field_names(self) -> None:
        ...
        self.assertIn('house_attr: Number(seed?.house_attr || 0)', html)
    ...
    def test_editor_app_preserves_manager_state_and_derives_relations(self) -> None:
        ...
        self.assertIn('stageIniRowKey: seed?.stageIniRowKey', html)
        self.assertIn('castle_scale: Number(seed?.castle_scale || 1)', html)
        self.assertIn('population: Number(seed?.population || 0)', html)
        self.assertIn('gold: Number(seed?.gold || 0)', html)
        self.assertIn('food: Number(seed?.food || 0)', html)
